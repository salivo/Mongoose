...
class DrawingCanvas(QWidget):
    ...
    def check_mouse_hover(self, px, py):
        best_match = None
        best_dist = float("inf")
        hit_threshold = 5.0 / (self.zoom * self.scale)
        for key, obj in self.objects.items():
            dist = float("inf")
            if isinstance(obj, Point):
                dist = math.hypot(px - obj.x, py - obj.y)
            elif isinstance(obj, Line):
                dist = self.dist_point_to_segment(
                    px, py, obj.p1.x, obj.p1.y, obj.p2.x, obj.p2.y
                )
            # If it's within our threshold AND closer than any other object we checked
    ...
    def draw_point(self, painter: QPainter, point: Point, is_hovered: bool):
        ...
        thickness = 2.5 if is_hovered else 2
    ...
    def map_to_logical(self, pos: QPointF) -> QPointF:
        ...
        # Reverse scale
        return QPointF(logical_x / self.scale, logical_y / self.scale)

    def dist_point_to_segment(self, px, py, x1, y1, x2, y2):
        l2 = (x2 - x1) ** 2 + (y2 - y1) ** 2
        if l2 == 0:
